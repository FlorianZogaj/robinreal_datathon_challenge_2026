#!/usr/bin/env python3
"""
Generate image captions for remaining listings using Claude Haiku.
Reads/writes the same raw_data/image_captions.json as generate_captions.py,
skipping any listing that already has a non-empty caption.

Usage:
    uv run python scripts/generate_captions_haiku.py
...
def _get_listings() -> list[dict]:
    if not DB_PATH.exists():
        raise SystemExit(f"DB not found at {DB_PATH}. Start the server once first.")
...
def _call_haiku(client: anthropic.Anthropic, image_bytes: bytes, mime: str) -> str:
...
async def _caption_one(
    ...
    async with sem:
        try:
            caption = await asyncio.to_thread(_call_haiku, client, image_bytes, mime)
